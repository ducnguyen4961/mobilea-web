...
export default function Threshold({ isOpen, onClose }: Props) {
  ...
  //Khai báo biến nhập PPFD và Lower Leaf Removal
  const [ppfd, setPpfd] = useState<{ [key: string]: number | string }>({});
  const [lowerleaf, setLowerleaf] = useState<{ [key: string]: number | string }>({});
    ...
    <div style={styles.overlay} onClick={handleClose}> {/* click ra ngoài hoặc vào trong phần component thì đều đóng component này lại */}
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}> {/* chặn lại vấn đề trên bằng cách khi người dùng ấn vào phần modal, còn ấn vào phần overlay thì đóng như thường */}
        <div style={{ position: "relative" }}>
          {/* Dropdown */}
          <div style={styles.selectBox} onClick={() => setOpen(!open)}> {/* đảo giá trị open để hiển thị phần dropdown  */}
            ...
            <div key={d} style={styles.card}>
              ...
              <div style={styles.params}>
                <div style={styles.field}>
                  <label style={styles.label}> PPFD閾値 (µmol/m²/s)</label>
                  <input style={styles.input} type="number" value={ppfd[d] || ""} onChange={(e) => setPpfd({...ppfd,[d]: e.target.value,})}/>
                </div>
                <div style={styles.field}>
                  <label style={styles.label}> 下葉除去閾値 </label>
                  <input style={styles.input} type="number" value={lowerleaf[d] || ""} onChange={(e) => setLowerleaf({...lowerleaf,[d]: e.target.value,})}/>
                </div>
              </div>
            </div>
            );
