...
export default function AdditionPhoton({ isOpen, onClose }: Props) {
  ...
  const [photon, setPhoton] = useState<{ [key: string]: string }>({}); 
  const [addphoton, setAddphoton] = useState<{ [key: string]: string }>({});
            ...
            <div key={d} style={styles.card}>
              ...
              <div style={styles.params}>
                <div style={styles.field}>
                  <label style={styles.label}> 基準光合成量 (mmolCO2/mBed)</label>
                  <input style={styles.input} type="number" value={photon[d] || ""} onChange={(e) => setPhoton({...photon,[d]: e.target.value,})}/>
                </div>
                <div style={styles.field}>
                  <label style={styles.label}> 合計光合成量 (mmolCO2/mBed) </label>
                  <input style={styles.input} type="number" value={addphoton[d] || ""} onChange={(e) => setAddphoton({...addphoton,[d]: e.target.value,})}/>
                </div>
                <div style={styles.field}>
